...
connect_psql() {
    for ((i = 0; i < $MAX_CREATE_ROLES_ATTEMPTS; i++)); do
        psql $CREATE_ROLES_SCRIPT_DB_NAME -U $CREATE_ROLES_SCRIPT_CREATOR_NAME -p $HAPROXY_PRIMARY_PORT -h $DB_HOSTNAME -b "$@" && break
        sleep $CREATE_ROLES_COOLDOWN_ON_FAILURE
    done
}
...
if [[ -z "$MONITORING_USERNAME" ]] || [[ -z "$MONITORING_USER_PASSWORD" ]]; then
    echo 'error: monitoring user password or username name is unknown'
else
    connect_psql -c "$(get_query_text "$MONITORING_USERNAME" "CREATE ROLE $MONITORING_USERNAME WITH LOGIN PASSWORD '$MONITORING_USER_PASSWORD'; GRANT CONNECT ON DATABASE $CREATE_ROLES_SCRIPT_DB_NAME TO $MONITORING_USERNAME; GRANT USAGE ON SCHEMA public TO $MONITORING_USERNAME; GRANT SELECT ON ALL TABLES IN SCHEMA public to $MONITORING_USERNAME;ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO $MONITORING_USERNAME; GRANT pg_monitor TO $MONITORING_USERNAME;")"
fi
